...
cursor.execute("CREATE TABLE IF NOT EXISTS tokens (username TEXT, token TEXT)")


def addTokenToDB(user, token):
    statement = "INSERT INTO tokens (username, token) VALUES (%s, %s)"
...
# Gets username associated with token
def getUserOfToken(token):
...
def replaceHomepage(cookie):
    ...
    if cookie != "":
        html = html.replace("<h3>Welcome!</h3>", "<h3>Welcome Back!</h3>")
...
# Returns bytearray of payload in frame
...
class MyTCPHandler(socketserver.BaseRequestHandler):
            ...
            while True:
                ...
                if received_data:
                    if client_id in clients:
                        self.handleSocketData(received_data)
                    else:
    ...
    def handleSocketData(self, received_data):
        # Decode data
        payload = parseFrame(received_data)
        print(payload)
        messageType = payload[9:payload.find(b',')-1].decode()

        # Get username and password
        user_pos = payload.find(b'username') + 11
        mes_pos = payload.find(b'"password')
        user = payload[user_pos: mes_pos - 2].decode()
        pw = payload[mes_pos + 12: payload.find(b'}') - 1]
        ...
        if messageType == "register":
            if not account:
                ...
                addAccountToDB(user, pw)
                # Register success
                result = b'{"alert":"1"}'
            else:
                # register failed: account already exists
                result = b'{"alert":"0"}'
        elif messageType == "login":
            print("logging in...")
            if account:
                user_pw = bcrypt.hashpw(pw, account[1].encode())
                if user_pw == account[0].encode():
                    # login success
                    # Generate auth token
                    token = secrets.token_hex(10)
                    while getUserOfToken(token):
                        token = secrets.token_hex(10)
                    addTokenToDB(user, token)
                    result = b'{"alert":"3","token":"' + token.encode() + b'"}'
            else:
                # login failed: Incorrect username or password
                result = b'{"alert":"2"}'
        print(result)
        self.request.sendall(createFrame(result))
        ...
        if request_line[0] == "GET":
            if path == "/":
                ...
                if "Cookie" in headers_dict:
                    cookies = headers_dict["Cookie"]
                    if cookies.find("visits") != -1:
                        addCookie = False
                    else:
                        cookies = ""
                self.createResponse_200("text/html", replaceHomepage(cookies), addCookie)
            elif path == "/functions.js":
                self.createResponse_200("text/javascript", readFile("functions.js"), False)
            elif path == "/style.css":
                self.createResponse_200("text/css", readFile("style.css"), False)
            elif path[:7] == "/images" and extension == ".jpg":
                data = readFile(path[1:])
                if data != -1:
                    self.createResponse_200("image/jpeg", data, False)
                else:
                    self.createResponse_404()
            elif path == "/websocket":
    ...
    def checkToken(self, headers_dict):
        print("checking token...")
        if "Cookie" in headers_dict:
            cookies = headers_dict["Cookie"] + ";"
            # print(cookies)
            token = cookies.find("UserToken")
            if token != -1:
                token = cookies.split("UserToken=")[1]
                # print(token)
                token = token[:token.find(";")]
                # print(token)
                hashedtoken = hashlib.sha1(token.encode()).hexdigest()
                # print(hashedtoken)
                user = getUserOfToken(hashedtoken)
                # print(user)
                if user != "":
                    result = b'{"alert":"4","username":"' + user[0].encode() + b'"}'
                    self.request.sendall(createFrame(result))

    # Handles the creation of all 200 HTTP responses
    # Assumes body is already encoded
    def createResponse_200(self, MIMEType, body, addCookie):
        ...
        # Adds visited cookie if addCookie == True
        if addCookie:
            noSniff += cookie
